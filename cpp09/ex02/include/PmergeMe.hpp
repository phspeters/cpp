...
class PmergeMe {
   ...
   private:
    ...
    void _merge_insertion_sort(Container& container, int pair_group_size);
    template <typename Container, typename Chain>
    void _merge_phase(Container& container, int pair_group_size);
    template <typename Container, typename Chain>
    void _create_chains(Container& container, int pair_group_size, Chain& main,
                        Chain& pend);
    template <typename Container, typename Chain>
    std::size_t _optimized_insertion(Chain& main, Chain& pend);
    template <typename Container, typename Chain>
    void _insert_remaining(std::size_t container_size, int pair_group_size,
                           Chain& main, Chain& pend,
                           std::size_t processed_pend_elements);
    template <typename Container, typename Chain>
    void _update_container(Container& container, Chain& main,
                           int pair_group_size);
...
template <typename Container, typename Chain>
void PmergeMe::_merge_insertion_sort(Container& container,
                                     int pair_group_size) {
    ...
    std::size_t processed_pend_elements =
        _optimized_insertion<Container, Chain>(main, pend);

    _insert_remaining<Container, Chain>(container.size(), pair_group_size, main,
                                        pend, processed_pend_elements);
...
void PmergeMe::_merge_phase(Container& container, int pair_group_size) {
    ...
    Iterator last_pair_end = get_iterator_at_offset(
        container.begin(), pair_group_size * 2 * pair_count);

    int jump = 2 * pair_group_size;
    for (Iterator it = first_pair_begin; it != last_pair_end;
         std::advance(it, jump)) {
        Iterator this_group_largest_element =
            get_iterator_at_offset(it, pair_group_size - 1);
        Iterator next_group_largest_element =
            get_iterator_at_offset(it, pair_group_size * 2 - 1);
...
template <typename Container, typename Chain>
void PmergeMe::_create_chains(Container& container, int pair_group_size,
                              Chain& main, Chain& pend) {
    typedef typename Container::iterator Iterator;
    ...
    for (int i = 0; i < group_count; i++) {
        int largest_element_offset =
            (pair_group_size * i) + pair_group_size - 1;
        Iterator current =
            get_iterator_at_offset(container.begin(), largest_element_offset);
...
std::size_t PmergeMe::_optimized_insertion(Chain& main, Chain& pend) {
    ...
    for (std::size_t jacob_index = 2; processed_pend_elements < pend.size();
         jacob_index++) {
        std::size_t curr_jacobsthal = _calculate_jacobsthal_number(jacob_index);
        std::size_t group_size = _calculate_jacobsthal_number(jacob_index) -
                                 _calculate_jacobsthal_number(jacob_index - 1);
        ...
        ChainIterator pend_it = get_iterator_at_offset(
            pend.begin() + processed_pend_elements, (elements_to_insert - 1));
        while (elements_to_insert--) {
            ChainIterator search_bound = get_iterator_at_offset(
                main.begin(), _calculate_jacobsthal_number(jacob_index) +
                                  processed_pend_elements - bound_adjustment);

            ChainIterator insertion_point = std::upper_bound(
                main.begin(), search_bound, *pend_it, _compare<Iterator>);
            ChainIterator inserted = main.insert(insertion_point, *pend_it);

            if (std::distance(main.begin(), inserted) ==
                static_cast<std::ptrdiff_t>(curr_jacobsthal +
                                            processed_pend_elements)) {
                bound_adjustment++;
            }
...
template <typename Container, typename Chain>
void PmergeMe::_insert_remaining(std::size_t container_size,
                                 int pair_group_size, Chain& main, Chain& pend,
                                 std::size_t processed_pend_elements) {
    typedef typename Container::iterator Iterator;
    typedef typename Chain::iterator ChainIterator;
    ...
    for (size_t index = processed_pend_elements; index < pend.size(); index++) {
        ...
        ChainIterator bound = get_iterator_at_offset(
            main.begin(), main.size() - pend.size() + index + is_odd);

        ChainIterator insertion_point =
            std::upper_bound(main.begin(), bound, *current, _compare<Iterator>);
        main.insert(insertion_point, *current);
    }
}

template <typename Container, typename Chain>
void PmergeMe::_update_container(Container& container, Chain& main,
                                 int pair_group_size) {
    typedef typename Container::iterator Iterator;
    typedef typename Chain::iterator ChainIterator;
...
template <typename Iterator>
void _swap_with_next_pair(Iterator rightmost_element_it, int pair_group_size) {
    Iterator current_pair_begin =
        get_iterator_at_offset(rightmost_element_it, -pair_group_size + 1);
    Iterator next_pair_begin =
        get_iterator_at_offset(current_pair_begin, pair_group_size);
