...
int BitcoinExchange::displayConvertions(std::string btc_database,
                                        std::string query_database) {
    if (_loadCsv(btc_database) || _convertQueries(query_database)) {
        return 1;
    }
...
int BitcoinExchange::_loadCsv(const std::string &filename) {
...
int BitcoinExchange::_convertQueries(const std::string &filename) {
    if (!_isValidFile(filename)) {
        return 1;
    }
    ...
    while (getline(file, line)) {
        ...
        if (!_isValidRecord(date)) {
            std::cout
                << "Error: records start at 2009-01-02 and end at 2022-03-29\n";
            continue;
        }
        ...
        if (!_isValidValue(value) || *end != '\0') {
            std::cout << "Error: invalid value => '" << value_str << "'"
...
void BitcoinExchange::_convertValue(
    ...
    std::map<std::string, double>::iterator data_it =
        _data.lower_bound(entry.first);
    ...
    std::cout << entry.first << " => " << entry.second << " = " << result
              << std::endl;
}

bool BitcoinExchange::_isValidFile(const std::string &filename) {
    if (filename.size() < 4 ||
        (filename.substr(filename.size() - 4) != ".txt" &&
         filename.substr(filename.size() - 4) != ".csv")) {
        std::cerr << "Error: file '" << filename
                  << "' does not have a .txt or .csv extension" << std::endl;
        return false;
    }
...
bool BitcoinExchange::_isValidDate(const std::string &date) {
    ...
    if (ss.fail() || sep1 != '-' || sep2 != '-' || day < 1 || month < 1 ||
        month > 12 || year < 0) {
        return false;
    }
